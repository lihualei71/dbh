...
compute_knots_mvgauss <- function( zstat, zminus, cor,
    ...
    if (navals > 1){
        ...
        thresh <- qnorm(pmin(1, weights * alpha * avals / n), lower.tail = FALSE)
    ...
    for (tail in 1:ntails){             # R tail first and then L tail if 2-sided
        ...
        for (k in 1:length(ids[[tail]])){
            ...
            if (navals > 1){
                ...
                thr <- qnorm(pmin(1, alpha * weights[i] * avals[thrids] / n), lower.tail = FALSE)      # which thresholds this coordinate crosses
            } else {
                thrids <- 1
